...
export const CardsView: React.FC<CardsViewProps> = ({ cards, teams, selectedTeamId }) => {
  ...
  const suspendedCount = cards.filter(c => c.status === 'Karantene').length;
  const warningCount = cards.filter(c => c.status.includes('Advarsel')).length;
  const totalYellows = cards.reduce((acc, c) => acc + c.yellowCards, 0);
  const totalReds = cards.reduce((acc, c) => acc + c.redCards, 0);
